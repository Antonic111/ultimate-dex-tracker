...
import { Calendar, AlertCircle, ChevronLeft, ChevronRight, Filter, Search, FileText, Zap, ChevronDown } from 'lucide-react';
...
const Changelog = () => {
  ...
  React.useEffect(() => { setCurrentPage(1); }, [filterType, searchTerm]);
  ...
  const getTypeIcon = (type) => {
    switch (type) {
      case 'feature': return <Zap className="changelog-icon feature" />;
      case 'fix': return <AlertCircle className="changelog-icon fix" />;
      default: return <Calendar className="changelog-icon" />;
    }
  };
        ...
        <section className="changelog-section">
          ...
          <div className="changelog-controls">
            ...
            <div className="filter-controls">
              <div className={`filter-button-wrap ${showFilterDropdown ? 'open' : ''}`} ref={filterButtonRef}>
                <button className="filter-button" onClick={() => setShowFilterDropdown(!showFilterDropdown)} aria-label="Filter updates">
                ...
                {showFilterDropdown && (
                  <div className="filter-dropdown">
                    <button className={`filter-option ${filterType === "all" ? "active" : ""}`} onClick={() => { setFilterType("all"); setShowFilterDropdown(false); }}>All Updates</button>
                    <button className={`filter-option ${filterType === "feature" ? "active" : ""}`} onClick={() => { setFilterType("feature"); setShowFilterDropdown(false); }}>New Features</button>
                    <button className={`filter-option ${filterType === "fix" ? "active" : ""}`} onClick={() => { setFilterType("fix"); setShowFilterDropdown(false); }}>Bug Fixes</button>
                  </div>
                )}
              </div>
            </div>
          ...
          {totalPages > 1 && (
            <div className="pagination-controls">
              <div className="pagination-info">Showing {startIndex + 1} of {filteredEntries.length} updates</div>
              ...
              <div key={index} className="changelog-entry">
                ...
                {entry.features && entry.features.length > 0 && (
                  <div className="changelog-section">
                    ...
                    <ul className="changelog-list">
                      {entry.features.map((feature, i) => <li key={i} className="changelog-item">{feature}</li>)}
                    </ul>
                  </div>
                )}
                ...
                {entry.changes && entry.changes.length > 0 && (
                  <div className="changelog-section">
                    ...
                    <ul className="changelog-list">
                      {entry.changes.map((change, i) => <li key={i} className="changelog-item">{change}</li>)}
                    </ul>
                  </div>
                )}
                ...
                {entry.fixes && entry.fixes.length > 0 && (
                  <div className="changelog-section">
                    ...
                    <ul className="changelog-list">
                      {entry.fixes.map((fix, i) => <li key={i} className="changelog-item">{fix}</li>)}
                    </ul>
                  </div>
                )}
              </div>
