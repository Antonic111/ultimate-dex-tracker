...
import "../../css/ConnectedAccounts.css";
...
export default function ConnectedAccounts() {
  ...
  return (
    <div className="setting-block">
      ...
      <p style={{ marginBottom: "12px", fontSize: "0.85rem", color: "var(--text-muted, #888)" }}>
        Link Google or Discord to sign in faster and manage multiple login methods for your account.
      </p>

      <div className="connected-accounts-list">
        {/* Google */}
        <div className="connected-account-row">
          <div className="connected-account-icon connected-account-icon--google">
            <GoogleIcon />
          </div>
          <div className="connected-account-info">
            ...
            {googleAccount ? (
              ...
              <div className="connected-account-status">Not connected</div>
            )}
          </div>
          <div className="connected-account-action">
            {googleAccount ? (
              <button
                type="button"
                className="connected-account-disconnect-btn"
                onClick={() => handleDisconnect("google")}
                disabled={actionLoading === "google" || loading}
              >
                {actionLoading === "google" ? "Disconnecting..." : "Disconnect"}
              </button>
            ) : (
              <button
                type="button"
                className="connected-account-connect-btn"
                onClick={() => handleConnect("google")}
                disabled={actionLoading === "google" || loading}
              >
                {actionLoading === "google" ? "Connecting..." : "Connect"}
              </button>
            )}
          </div>
        </div>
        ...
        {/* Discord */}
        <div className="connected-account-row">
          <div className="connected-account-icon connected-account-icon--discord">
            <DiscordIcon />
          </div>
          <div className="connected-account-info">
          ...
          <div className="connected-account-action">
            {discordAccount ? (
              <button
                ...
                disabled={actionLoading === "discord" || loading}
              >
                {actionLoading === "discord" ? "Disconnecting..." : "Disconnect"}
              </button>
            ) : (
              <button
                type="button"
      ...
      <div className="connected-accounts-notice">
        <strong>Security tip:</strong> You must always have at least one password or connected account active so you don't lose access to your dex.
      </div>
    </div>
  );
}
